...
{ "schemaVersion": 1,
  "fields": [
    ...
    { "id": "service", "type": "radio", "label": "Service", "options": [
        ...
        { "value": "opt_9c1e", "label": "Deep clean", "price": 4, "image": 123 } ] },
...
| toggle | `"1"` when on, `""` when off (ConditionEditor renders an On/Off picker — users never type these literals) |
...
- Capabilities: all admin REST routes + pages require `manage_options`; the quote endpoint is public with the abuse controls defined in §10 (honeypot, per-IP rate limit, payload size cap, strict sanitization — deliberately no nonce).
