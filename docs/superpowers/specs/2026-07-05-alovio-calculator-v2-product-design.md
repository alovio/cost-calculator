...
- **Entries surfaces:** the entry detail modal renders a repeater as grouped rows ("Room 1: Area 20, Rate Standard — $120.00", one line per row). `CsvExporter` flattens each repeater to ONE cell: rows joined with `" | "` in the form `Room 1: area=20, rate=Standard ($120.00)` (keys = child ids, values = display labels) — RFC-4180 quoting and the existing injection guard apply unchanged. The notification email uses the same per-row lines as the detail modal.
